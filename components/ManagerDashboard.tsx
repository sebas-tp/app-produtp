...
import { 
  ...
  Scale, AlertTriangle, Clock, AlertOctagon, Timer
} from 'lucide-react';
...
export const ManagerDashboard: React.FC = () => {
  ...
  // --- UI / PESTAÑAS ---
  // AHORA TENEMOS 3 PESTAÑAS: metrics (original), efficiency (ocio), audit (tango)
  ...
  // --- CONFIGURACIÓN DE EFICIENCIA (PESTAÑA 2) ---
  const [workingDays, setWorkingDays] = useState<number>(17); // Default ej: 17 días
  const [shiftHours, setShiftHours] = useState<number>(8.5);  // Default 8.5hs
  ...
  const MASTER_PASSWORD = "admin"; // Tu clave original (o la que hayas puesto)
  ...
  // DATOS PARA RANKING Y EFICIENCIA
  ...
  .map(stat => {
      ...
      return { 
          ...stat, 
          percentage: (stat.points / dailyTarget) * 100, // % vs Meta Diaria (para visualización simple)
          potentialPoints,
          idlePoints,
          idlePercentage
      };
  ...
  const handleFullReportPDF = async () => {
    ...
    // PDF SEGÚN PESTAÑA ACTIVA
    if (activeTab === 'audit') {
        const auditData = getAuditData();
        doc.text("AUDITORÍA DE STOCK", 14, 45);
        ...
        doc.text("REPORTE DE OCIO Y EFICIENCIA", 14, 45);
        doc.text(`Base: ${workingDays} días x ${shiftHours} hs`, 14, 52);
        autoTable(doc, { startY: 55, head: [['Operario', 'Real', 'Potencial', 'Ocio (Pts)', '% Ocio']], body: rankingStats.map(s => [s.name, s.points.toFixed(0), s.potentialPoints.toFixed(0), s.idlePoints.toFixed(0), s.idlePercentage.toFixed(1) + '%']) });
    } else {
        doc.text("RESUMEN DE MÉTRICAS", 14, 45);
        autoTable(doc, { startY: 50, head: [['Fecha', 'Real', 'Meta', '%']], body: dailyTrend.map(d => [d.name, d.points.toLocaleString(), dailyTarget.toLocaleString(), ((d.points/dailyTarget)*100).toFixed(0)+'%']) });
    }
      ...
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 bg-white p-4 rounded-xl shadow-sm border border-slate-100">
        ...
        <div className="flex flex-wrap gap-3 items-center">
            ...
            <div className="flex gap-2">
                <button onClick={handleFullReportPDF} className="p-2 bg-slate-800 text-white rounded-lg hover:bg-slate-700" title="PDF"><FileText className="w-5 h-5" /></button>
                ...
                <button onClick={handleEngineeringAccess} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all ${isAuthorized ? 'bg-indigo-600 text-white' : 'bg-slate-200 text-slate-500'}`}>{isAuthorized ? <BrainCircuit className="w-4 h-4" /> : <Lock className="w-4 h-4" />}</button>
            </div>
        </div>
      </div>

      {/* ================================================================================== */}
      {/* PESTAÑA 1: MÉTRICAS ORIGINALES (RESTORED) */}
      {/* ================================================================================== */}
      {activeTab === 'metrics' && (
        <div className="space-y-6 animate-in fade-in">
            {/* KPI CARDS CON EDITOR DE META (VUELVE EL LÁPIZ) */}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                {/* RANKING SIMPLE (SIN COLUMNAS RARAS) */}
      ...
      {activeTab === 'efficiency' && (
         <div className="animate-in fade-in space-y-6">
            <div className="bg-indigo-50 p-6 rounded-xl shadow-md border-l-4 border-indigo-600">
                <h3 className="text-xl font-bold text-slate-800 flex items-center gap-2"><Timer className="w-6 h-6 text-indigo-600"/> Calculadora de Capacidad</h3>
                <p className="text-slate-600 text-sm mt-1 mb-4">Defina los parámetros del periodo para calcular el tiempo ocioso y la eficiencia real de la planta.</p>
                
                <div className="flex flex-wrap gap-6 bg-white p-4 rounded-lg shadow-sm border border-indigo-100">
                     <div><label className="text-[10px] text-indigo-500 font-bold block mb-1">DÍAS HÁBILES</label><input type="number" value={workingDays} onChange={e => setWorkingDays(Number(e.target.value))} className="w-24 bg-slate-50 border border-slate-300 rounded px-3 py-2 text-lg font-bold text-slate-800 focus:ring-2 focus:ring-indigo-500 outline-none"/></div>
                     ...
                     <div className="flex flex-col justify-end pb-2"><span className="text-xs text-slate-400 font-medium">x 3600 pts/hora =</span></div>
                     <div><label className="text-[10px] text-slate-400 font-bold block mb-1">CAPACIDAD IDEAL</label><div className="bg-indigo-100 px-4 py-2 rounded text-lg font-black text-indigo-700 border border-indigo-200">{(workingDays * shiftHours * pointsPerHour).toLocaleString()} pts</div></div>
                </div>
            </div>

            <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                 <div className="px-6 py-4 border-b border-slate-100"><h3 className="font-bold text-slate-700">Reporte de Ocio por Operario</h3></div>
                    ...
                    <table className="w-full text-sm text-left">
                        <thead className="bg-slate-50 text-xs text-slate-500 uppercase sticky top-0">
                            <tr>
                                ...
                                <th className="px-6 py-4 text-right text-red-600 bg-red-50/50">Pts Ociosos</th>
                                <th className="px-6 py-4 text-center bg-red-50/50">% Ocio</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100">
                            {rankingStats.map((stat, idx) => (
                                <tr key={stat.name} className="hover:bg-slate-50">
                                    ...
                                    <td className="px-6 py-4 text-right font-bold text-red-600 bg-red-50/30">{stat.idlePoints.toLocaleString('es-AR')}</td>
                                    <td className="px-6 py-4 text-center bg-red-50/30">
                                        {stat.idlePoints > 0 ? (
                                            <span className={`px-2 py-1 rounded text-xs font-bold border ${stat.idlePercentage > 20 ? 'bg-red-100 text-red-700 border-red-200' : 'bg-yellow-100 text-yellow-700 border-yellow-200'}`}>{stat.idlePercentage.toFixed(1)}%</span>
                                        ) : <span className="text-green-600 text-xs font-bold">Superávit</span>}
                                    </td>
                                </tr>
                            ))}
                        </tbody>
            ...
            <div className="bg-slate-50 p-4 rounded-lg border border-slate-200 flex gap-3 items-center text-xs text-slate-600">
                ...
                <p>El "Tiempo Ocioso" se calcula restando la producción real a la capacidad teórica del operario en el periodo seleccionado.</p>
            </div>
         </div>
      )}
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left">
                        ...
                        <tbody className="divide-y divide-slate-100">
                            ...
                            {getAuditData().length === 0 && <tr><td colSpan={7} className="text-center py-10 text-slate-400">Sin datos.</td></tr>}
                        </tbody>
                    </table>
                </div>
            </div>
